...
import { defaults, Options, cleanUrl } from './conversionOptions';
import { cleanMarkdown } from './cleanup';
export { defaults, Options, labels, trackingDescription, trackingParameters, cleanUrl } from './conversionOptions';
...
function convertContent(input: { html?: string; text?: string }, options: Options = defaults): string {
    if (!input.html) {
        if (options.removeTextStyling) return cleanMarkdown(input.text || '', options);
    ...
    service.addRule('highlight', { filter: 'mark', replacement: content => `==${content}==` });
